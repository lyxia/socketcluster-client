...
SCClientSocket.prototype._privateDataHandlerMap = {
  '#publish': function (data) {
    ...
    if (isSubscribed) {
      this.channels[undecoratedChannelName].write(data.data);
    }
  },
...
SCClientSocket.prototype.connect = SCClientSocket.prototype.open = function () {
  ...
  if (this.state === this.CLOSED) {
    ...
    this.transport.on('receiveData', (event, data) => {
      this._onSCReceiveData(event, data);
    });

    this.transport.on('receiveRPC', (event, data, response) => {
      this._onSCReceiveRPC(event, data, response);
    });
  }
};
...
// TODO 2: Cannot count llisteners so what will happen if there are no error listeners?
...
SCClientSocket.prototype.emit = function (event, data) {
...
SCClientSocket.prototype._onSCReceiveData = function (event, data) {
  var handler = this._privateDataHandlerMap[event];
  if (handler) {
    handler.call(this, data);
  } else {
    var receiver = this.receivers[event];
    if (receiver) {
      receiver.write(data);
    }
  }
...
SCClientSocket.prototype._onSCReceiveRPC = function (event, data, response) {
  var handler = this._privateRPCHandlerMap[event];
  if (handler) {
    handler.call(this, data, response);
  } else {
    var procedure = this.procedures[event];
    if (procedure) {
      procedure.write({
        data: data,
        end: (data) => {
...
SCClientSocket.prototype.receiver = function (eventName) {
...
SCClientSocket.prototype.destroyReceiver = function (eventName) {
  delete this.receivers[eventName];
};

SCClientSocket.prototype.prodecure = function (procedureName) {
...
SCClientSocket.prototype.destroyProcedure = function (procedureName) {
...
SCClientSocket.prototype.destroyListener = function (eventName) {
