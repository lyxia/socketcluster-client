...
SCClientSocket.prototype.connect = SCClientSocket.prototype.open = function () {
  ...
  if (this.state === this.CLOSED) {
    ...
    this.transport.on('inboundTransmit', (event, data) => {
      this._onSCInboundTransmit(event, data);
    });

    this.transport.on('inboundInvoke', (event, data, response) => {
      this._onSCInboundInvoke(event, data, response);
    });
  }
};
...
SCClientSocket.prototype._onSCError = function (err) {
  var listenerCount = Object.keys(this.listeners || {}).length;
  if (listenerCount < 1) {
    setTimeout(() => {
      throw err;
    }, 0);
