...
SCClientSocket.prototype.connect = async function () {
  if (!this.active) {
...
SCClientSocket.prototype.reconnect = async function (code, data) {
  this.disconnect(code, data);
  this.connect();
  await wait(0);
};

SCClientSocket.prototype.disconnect = async function (code, data) {
  code = code || 1000;
  ...
  await wait(0); // TODO 2 or maybe await disconnect once() instead? Also do this for other methods.
};

SCClientSocket.prototype.destroy = async function (code, data) {
...
SCClientSocket.prototype.subscribe = function (channelName, options) {
  ...
  let channelEventStream = this._channelEventDemux.stream(channelName);
